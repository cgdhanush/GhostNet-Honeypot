...
FT_LOGGING_CONFIG = {
    ...
    "handlers": {
        "console": {
            "class": "honeypot.loggers.ft_rich_handler.FtRichHandler",
            "formatter": "basic",
        },
    },
    "root": {
...
def _set_log_levels(
    ...
    third_party_loggers = {
        "honeypot": logging.INFO if verbosity <= 1 else logging.DEBUG,
...
def _create_log_config() -> dict[str, Any]:
    ...
    for handler_config in log_config.get("handlers", {}).values():
        if handler_config.get("class") == "honeypot.loggers.ft_rich_handler.FtRichHandler":
