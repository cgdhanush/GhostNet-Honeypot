...
FT_LOGGING_CONFIG = {
    ...
    "handlers": {
        "console": {
            "class": "ghostnet.loggers.ft_rich_handler.FtRichHandler",
            "formatter": "basic",
        },
    },
    "root": {
...
def _set_log_levels(
    ...
    third_party_loggers = {
        "ghostnet": logging.INFO if verbosity <= 1 else logging.DEBUG,
        ...
        "ccxt.base.broker": logging.INFO if verbosity <= 2 else logging.DEBUG,
...
def _create_log_config() -> dict[str, Any]:
    # Get log_config from user config or use default
    log_config = copy.deepcopy(FT_LOGGING_CONFIG)
    ...
    for handler_config in log_config.get("handlers", {}).values():
        if handler_config.get("class") == "ghostnet.loggers.ft_rich_handler.FtRichHandler":
            ...
            try:
                ...
                raise Exception(
                    ...
                    "directories. If you're running honeypot using docker, you see this error "
